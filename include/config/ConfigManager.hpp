...
public:
    ...
    /// @brief Construct a ConfigManager
    /// @param name The name of the configuration
    /// @param store The configuration store
    /// @param codec The JSON codec
    /// @param extra_validate Additional validation function, optional
    ...
    /// @brief Load from store, migrate, validate, apply.
    /// @return Result<void> indicating success or failure
    Result<void> load()
    {
        auto r = store_->read(name_);
        if (!r)
            return Result<void>::fail(name_, "read failed");
        ...
        if (!cfgR)
            return Result<void>::fail(cfgR.error.where, cfgR.error.msg);
    ...
    Result<void> apply_json(std::string_view json, bool persist)
    {
        ...
        if (persist)
        {
            ...
            if (!enc)
                return Result<void>::fail(enc.error.where, enc.error.msg);
    ...
    /// @brief Get the current configuration (thread-safe)
    ...
    /// @brief Observer interface for config changes
    /// @tparam T The configuration type
    /// @note Observers are called without holding the ConfigManager lock
